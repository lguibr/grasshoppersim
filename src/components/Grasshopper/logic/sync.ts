...
export const syncGrasshopperState = (
  ...
  materials.abdomen.color.lerpColors(baseColors.red, baseColors.abdomen, healthRatio);
  materials.thorax.color.lerpColors(baseColors.red, baseColors.thorax, healthRatio);
  ...
  useSimulationStore.updateStat(id, {
    id, health: s.health, action, isBaby: s.growth < 1, traits: s.traits,
    name: s.name, birthTime: s.birthTime
  });
  s.lastSyncTime = t;
};
