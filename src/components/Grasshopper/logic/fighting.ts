  ...
  if (t - (s as any).lastFightCheckTime < 0.1) {
    if ((s as any).wasFighting) {
      ...
      if ((s as any).lastFightOtherId) {
        const otherStat = useSimulationStore.stats.get(
          (s as any).lastFightOtherId,
        );
      ...
      if (refs.leftFrontLeg.current)
        refs.leftFrontLeg.current.rotation.x = 0.3 + Math.sin(t * 50) * 0.5;
      if (refs.rightFrontLeg.current)
        refs.rightFrontLeg.current.rotation.x = 0.3 + Math.cos(t * 50) * 0.5;
      if (refs.leftWing.current)
        refs.leftWing.current.rotation.set(
          Math.PI / 2 - 0.5,
          Math.sin(t * 60) * 0.5,
          -0.8 + Math.sin(t * 60),
        );
      if (refs.rightWing.current)
        refs.rightWing.current.rotation.set(
          Math.PI / 2 - 0.5,
          -Math.sin(t * 60) * 0.5,
          0.8 - Math.sin(t * 60),
        );
