...
describe("Grasshopper Animation Logic", () => {
  ...
  it("13. Jump peaks halfway through interpolation (progress=0.6)", () => {
    ...
    expect(mockRefs.group.current?.position.length()).toBeGreaterThan(
      mockState.jumpHeight * 0.9,
    );
  });
  ...
  it("16. resetAnimation resets wings properly", () => {
    ...
    expect(mockRefs.leftWing.current?.rotation.x).toBeCloseTo(
      Math.PI / 2 - 0.25,
    );
    expect(mockRefs.rightWing.current?.rotation.x).toBeCloseTo(
      Math.PI / 2 - 0.25,
    );
  });
