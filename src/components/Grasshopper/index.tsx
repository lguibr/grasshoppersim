...
export const Grasshopper = React.memo(({ id, position, isBaby, parentTraits, positionsRef, foodsRef, targetsRef, onFoodEaten, onDie, onSpawnEgg }: Props) => {
  const { refs, shape, materials } = useGrasshopper(id, position, isBaby, parentTraits, positionsRef, foodsRef, targetsRef, onFoodEaten, onDie, onSpawnEgg);
  ...
  return (
    <group ref={refs.group} position={position}>
      ...
      <group position={[0.22, 0.8, 0.65]} rotation={[-Math.PI / 2 - 0.2, 0, 0]}>
