...
export const InstancedFoods = ({ foodsRef }: { foodsRef: React.MutableRefObject<Map<number, FoodData>> }) => {
  ...
  useFrame((_, delta) => {
    ...
    foodsRef.current.forEach((data) => {
      ...
      if (data.type === 'egg') {
        ...
        // Plant
        dummyLeaves.position.set(0, 0.5, 0);
        ...
        if (plantMainMesh.current) plantMainMesh.current.setMatrixAt(plantCount, finalMatrix);
        if (plantSide1Mesh.current) plantSide1Mesh.current.setMatrixAt(plantCount, finalMatrix);
        if (plantSide2Mesh.current) plantSide2Mesh.current.setMatrixAt(plantCount, finalMatrix);
  ...
  const geometries = useMemo(() => {
    const egg = new THREE.SphereGeometry(0.4, 16, 16);
    // Grass blades
    const main = new THREE.ConeGeometry(0.1, 1.2, 3);
    main.translate(0, 0.6, 0);
    const side1 = new THREE.ConeGeometry(0.08, 0.8, 3);
    side1.rotateZ(-0.4);
    side1.rotateY(0.5);
    side1.translate(0.2, 0.4, 0);
    const side2 = new THREE.ConeGeometry(0.08, 0.9, 3);
    side2.rotateZ(0.3);
    side2.rotateY(-0.8);
    side2.translate(-0.2, 0.45, 0);
    return { egg, main, side1, side2 };
  }, []);

  return (
    <group>
      <instancedMesh ref={eggMesh} args={[geometries.egg, undefined, maxItems]} castShadow receiveShadow>
        <meshStandardMaterial color="#fef08a" emissive="#fef08a" emissiveIntensity={0.2} roughness={0.4} />
      </instancedMesh>
      <instancedMesh ref={plantMainMesh} args={[geometries.main, undefined, maxItems]} castShadow>
        <meshStandardMaterial color="#22c55e" emissive="#22c55e" emissiveIntensity={0.2} roughness={0.9} />
      </instancedMesh>
      <instancedMesh ref={plantSide1Mesh} args={[geometries.side1, undefined, maxItems]} castShadow>
        <meshStandardMaterial color="#16a34a" emissive="#16a34a" emissiveIntensity={0.2} roughness={0.9} />
      </instancedMesh>
      <instancedMesh ref={plantSide2Mesh} args={[geometries.side2, undefined, maxItems]} castShadow>
        <meshStandardMaterial color="#15803d" emissive="#15803d" emissiveIntensity={0.2} roughness={0.9} />
      </instancedMesh>
    </group>
  );
};
