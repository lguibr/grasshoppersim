...
export const useEggHatcher = (
  ...
  addCrickets: (newCrickets: { id: number, pos: [number, number, number], isBaby: boolean, parentTraits?: GrasshopperTraits }[]) => void
) => {
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const now = Date.now();
      let hatched = false;
      const newCrickets: { id: number, pos: [number, number, number], isBaby: boolean, parentTraits?: GrasshopperTraits }[] = [];
      
      foodsRef.current.forEach((food, id) => {
        if (food.type === 'egg' && food.spawnTime && now - food.spawnTime > 10000) {
          foodsRef.current.delete(id);
          hatched = true;
          newCrickets.push({ id: generateId(), pos: food.position, isBaby: true, parentTraits: food.parentTraits });
        }
      });
