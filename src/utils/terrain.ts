...
export const getGroundHeight = (pos: THREE.Vector3) => {
  ...
  // Overall shape warp
  noise +=
    Math.sin(dir.x * 1.5) * Math.cos(dir.y * 1.5) * currentRoughness * 0.1;

  // Base low frequency structure (continents and oceans)
  noise +=
    Math.sin(dir.x * 2.5 + Math.sin(dir.y * 3.0)) *
    Math.cos(dir.z * 2.5) *
    currentRoughness *
    0.15;
  noise +=
    Math.cos(dir.x * 4.0 + dir.z * 3.0) *
    Math.sin(dir.y * 4.0) *
    currentRoughness *
    0.08;
  ...
  noise +=
    Math.sin(dir.x * 12.0) *
    Math.cos(dir.y * 11.0 + dir.z * 13.0) *
    currentRoughness *
    0.03;

  // High frequency roughness
  noise +=
    Math.sin(dir.x * 30.0 + dir.y * 25.0) *
    Math.cos(dir.z * 28.0) *
    currentRoughness *
    0.01;
  noise +=
    Math.sin(dir.x * 60.0 - dir.z * 50.0) *
    Math.sin(dir.y * 55.0) *
    currentRoughness *
    0.005;
...
export const getHeadingToPoint = (from: THREE.Vector3, to: THREE.Vector3) => {
  ...
  if (north.lengthSq() < 0.001)
    north = new THREE.Vector3(0, 0, -1).projectOnPlane(up).normalize();
...
export const getPointFromHeadingAndDistance = (
  from: THREE.Vector3,
  heading: number,
  distance: number,
